...
$query=mysqli_query($koneksi, "SELECT * FROM status_klaim WHERE ID_Statusklaim LIKE '%$keyword%' OR Status_Klaim LIKE '%$keyword%'")
...
<form class="form-horizontal" role="search" method="post" action="index.php?halaman=pencarianstatusklaim">
  ...
  <tbody>
    <?php $nomor=1;?>
    <?php $ambil=$koneksi->query("SELECT * FROM status_klaim WHERE ID_Statusklaim LIKE '%$keyword%' OR Status_Klaim LIKE '%$keyword%'");?>
