...
<center><h3> Laporan Cabang Bank Berdasarkan Berat Total Jenis Sampah dari <?php echo $tgl_mulai?> hingga <?php echo $tgl_selesai?></h3>
</center><hr>
	<form method="post">
		<div class="row">
			<div class="col-md-2">
			...
			<div class="col-md-2">
        		<div class="form-group">
          			...
          			<select class="form-control" name="banksampah">
            			<option value="Nama_Bank">--Pilih Cabang Bank--</option>
            			<?php $ambil=$koneksi->query("SELECT * FROM cabang_bank_sampah ");
                		while($percabangbank=$ambil->fetch_assoc()){?>
                		<option value="<?php echo $percabangbank['ID_Cabang_Bank'];?>"><?php echo $perjenissampah['Nama_Bank'];?></option>
              			<?php }?>
          			</select>
        		</div>
      		</div>
      		<div class="col-md-2">
        		<div class="form-group">
          			...
          			<select class="form-control" name="ID_Jenissampah">
                		...
                		while($perjenissampah=$ambil->fetch_assoc()){?>
                		<option value="<?php echo $perjenissampah['ID_Jenis_Sampah'];?>"><?php echo $perjenissampah['Jenis_Sampah'];?></option>
              			<?php }?>
          			</select>
        		</div>
      		</div>
			...
			<div class="col-md-1">
      			<div class="form-group">
      				<label>&nbsp;</label> 
        			<a href="index.php?halaman=semualaporancabangbankberdasarkanberattotaljenissampah" class="btn btn-warning square-btn-adjust"><i class="fa fa-folder-open-o"></i> Lihat Semua Laporan
        			</a>
      			</div> 
      		</div> 
		</div>
...
<!-- End Navbar -->
    <div class="card shadow" style="margin-top:22px">
      <div class="card-header border-4">        
        <h4 class="mb-0">Tabel Laporan Cabang Bank Berdasarkan Berat Total Jenis Sampah</h4>
      </div>
...
?>
 ...
 <?php $ambil=mysqli_query($koneksi,"SELECT * , sum(Sub_Total) AS Sub_Total, count(transaksi_setor.ID_Nota_Setor) AS Total_Transaksi_Setor FROM transaksi_setor 
 INNER JOIN sampah ON transaksi_setor.ID_Sampah = sampah.ID_Sampah JOIN jenis_sampah ON sampah.ID_Jenis_Sampah=jenis_sampah.ID_Jenis_Sampah 
 INNER JOIN nota_setor ON transaksi_setor.ID_Nota_Setor=nota_setor.ID_Nota_Setor JOIN cabang_bank_sampah ON nota_setor.ID_Cabang_Bank=cabang_bank_sampah.ID_Cabang_Bank JOIN status_setor ON nota_setor.ID_Statussetor=status_setor.ID_Statussetor 
WHERE ID_Jenis_Sampah='$IDJenissampah' AND ID_Cabang_Bank='$IDCabangbank' AND nota_setor.ID_Statussetor='SS000001' AND Tanggal_setor BETWEEN '$dt1' AND'$dt2' GROUP BY cabang_bank_sampah.ID_Cabang_Bank ORDER BY Sub_Total DESC LIMIT 5");
	}
?>
			...
			<?php while($pecah=mysqli_fetch_array($ambil)){?>			
      		...
      		<?php } ?>
      		<tr>
        		<td colspan="4"><center> Total Berat Per Periode </td>
