...
if(isset($_POST["proses"]))
{
   ...
   $ambil=$koneksi->query("SELECT *, sum(QTY) AS QTY, count(transaksi_gift.ID_Nota_Gift) AS Total_Transaksi_gift FROM transaksi_gift INNER JOIN hadiah ON transaksi_gift.ID_Gift = hadiah.ID_Gift INNER JOIN nota_gift ON transaksi_gift.ID_Nota_Gift=nota_gift.ID_Nota_Gift JOIN cabang_bank_sampah ON nota_gift.ID_Cabang_Bank=cabang_bank_sampah.ID_Cabang_Bank JOIN status_klaim ON nota_gift.ID_Statusklaim=status_klaim.ID_Statusklaim WHERE Tanggal_Klaim BETWEEN '$tgl_mulai' AND'$tgl_selesai'");
...
<center><h3> Laporan Total Hadiah Berdasarkan Hadiah dari <?php echo $tgl_mulai?> hingga <?php echo $tgl_selesai?></h3>
</center><hr>
	<form method="post">
		<div class="row">
			<div class="col-md-3">
			...
			<div class="col-md-1">
      			<div class="form-group">
      				<label>&nbsp;</label> 
        			<a href="index.php?halaman=semualaporantotalhadiah" class="btn btn-warning square-btn-adjust"><i class="fa fa-folder-open-o"></i> Lihat Semua Laporan
        			</a>
      			</div> 
      		</div> 
		</div>
...
<!-- End Navbar -->
  ...
  <thead>
    <a href="index.php?halaman=laporantotalhadiah" class="btn btn-success square-btn-adjust" style="margin-bottom:16px"><i class="fa fa-refresh"></i> Refresh </a> 
...
?>
	<script language="JavaScript">
        ...
        document.location='index.php?halaman=laporantotalhadiah';
    </script>	
...
?>
	<br>
<?php
   $ambil=mysqli_query($koneksi,"SELECT *, sum(QTY) AS QTY, count(transaksi_gift.ID_Nota_Gift)
    AS Total_Transaksi_Klaim FROM transaksi_gift 
   INNER JOIN hadiah ON transaksi_gift.ID_Gift = hadiah.ID_Gift 
   INNER JOIN nota_gift ON transaksi_gift.ID_Nota_Gift=nota_gift.ID_Nota_Gift JOIN
    cabang_bank_sampah ON nota_gift.ID_Cabang_Bank=cabang_bank_sampah.ID_Cabang_Bank 
	JOIN status_klaim ON nota_gift.ID_Statusklaim=status_klaim.ID_Statusklaim 
   WHERE Nama_Gift='$_POST[ID_Hadiah]' AND nota_gift.ID_Statusklaim='SK000001' AND 
   Tanggal_Klaim BETWEEN '$dt1' AND'$dt2' GROUP BY hadiah.ID_Gift ORDER BY QTY DESC LIMIT 5");
	}
?>
			<?php $Total_Hadiah=0; ?>
			<?php $nomor=1; ?>
