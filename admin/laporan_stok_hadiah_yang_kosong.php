...
if(isset($_POST["proses"]))
{
   ...
   $ambil=$koneksi->query("SELECT * FROM transaksi_stok_hadiah INNER JOIN hadiah ON transaksi_stok_hadiah.ID_Gift = hadiah.ID_Gift INNER JOIN cabang_bank_sampah ON transaksi_stok_hadiah.ID_Cabang_Bank=cabang_bank_sampah.ID_Cabang_Bank");
...
<center><h3> Laporan Stok Hadiah Kosong </h3>
</center><hr>
    <form method="post">
        <div class="row">
            <div class="col-md-2">
...
<!-- End Navbar -->
    <div class="card shadow" style="margin-top:22px">
      <div class="card-header border-4">        
        <h4 class="mb-0">Tabel Laporan Stok Hadiah kosong <?php echo $cabangbank?></h4>
      </div>
...
?>
<?php
   $ambil=mysqli_query($koneksi,"SELECT * FROM transaksi_stok_hadiah INNER JOIN hadiah ON transaksi_stok_hadiah.ID_Gift = hadiah.ID_Gift 
   INNER JOIN cabang_bank_sampah ON transaksi_stok_hadiah.ID_Cabang_Bank=cabang_bank_sampah.ID_Cabang_Bank 
   WHERE Nama_Bank='$cabangbank' AND Stok_Hadiah='0' GROUP BY hadiah.ID_Gift ORDER BY Stok_Hadiah DESC LIMIT 5");
?>
            <?php $nomor=1; ?>
