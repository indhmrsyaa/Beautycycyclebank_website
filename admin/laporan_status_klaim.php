...
if(isset($_POST["proses"]))
{
	...
	$ambil= $koneksi->query("SELECT * FROM nota_gift
...
<center><h3> Laporan Status Klaim dari <?php echo $tgl_mulai?> hingga <?php echo $tgl_selesai?></h3>
</center><hr>
	<form method="post">
		<div class="row">
			<div class="col-md-3">
			...
			<div class="col-md-2">
        		<div class="form-group">
          			...
          			<select class="form-control" name="ID_Status_Klaim">
            			<option value="status_klaim">--Pilih Status Klaim--</option>
            			<?php $ambil=$koneksi->query("SELECT * FROM status_klaim ");
                		while($perstatusklaim=$ambil->fetch_assoc()){?>
                		<option value="<?php echo $perstatusklaim['Status_Klaim'];?>"><?php echo $perstatusklaim['Status_Klaim'];?></option>
              			<?php }?>
          			</select>
        		</div>
      		</div>
			<div class="col-md-1">
				...
				<div class="col-md-1">
      				<div class="form-group">
      					<label>&nbsp;</label> 
        				<a href="index.php?halaman=semualaporanstatusklaim" class="btn btn-warning square-btn-adjust"><i class="fa fa-folder-open-o"></i> Lihat Semua Laporan
        			</a>
      			</div> 
      		</div>  
		</div>
...
<thead>
    <a href="index.php?halaman=laporanstatusklaim" class="btn btn-success square-btn-adjust" style="margin-bottom:16px"><i class="fa fa-refresh"></i> Refresh </a> 
...
<?php
	...
	INNER JOIN status_klaim ON nota_gift.ID_Statusklaim = status_klaim.ID_Statusklaim 
	WHERE Status_Klaim='$_POST[ID_Status_Klaim]' AND Tanggal_Klaim BETWEEN '$dt1' AND'$dt2' GROUP BY nota_gift.ID_Statusklaim ORDER BY jumlah_status_klaim DESC LIMIT 5");
	}
?>
        <?php
        $nomor=1;
